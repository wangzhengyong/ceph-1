...
namespace librbd {
  ...
  */
  ...
  ssize_t RBD::AioCompletion::get_return_value()
  {
    ...
    ssize_t RBD::AioCompletion::get_mismatch_offset()
  {
    librbd::io::AioCompletion *c = (librbd::io::AioCompletion *)pc;
    return c->get_mismatch_offset();
  }
  ...
  */
  ...
  ssize_t Image::compare_and_write(uint64_t ofs, size_t len, ceph::bufferlist &cmp_bl,
                            ceph::bufferlist& bl, uint64_t *mismatch_off, int op_flags)
  {
    ImageCtx *ictx = (ImageCtx *)ctx;
    tracepoint(librbd, compare_and_write_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
  ...
  int Image::aio_compare_and_write(uint64_t off, size_t len, ceph::bufferlist& cmp_bl,
      ceph::bufferlist& bl, RBD::AioCompletion *c, int op_flags)
  {
    ...
    ictx->io_work_queue->aio_compare_and_write(get_aio_completion(c), off, len, bufferlist{cmp_bl},
                                               bufferlist{bl}, op_flags, false);
...
extern "C" ssize_t rbd_compare_and_write(rbd_image_t image, uint64_t ofs, size_t len,
                                   const char *cmp_buf, const char *buf, uint64_t *mismatch_off, int op_flags)
{
  ...
  int r = ictx->io_work_queue->compare_and_write(ofs, len, std::move(cmp_bl), std::move(bl), mismatch_off, op_flags);
  tracepoint(librbd, compare_and_write_exit, r);
  return r;
}
...
extern "C" ssize_t rbd_aio_compare_and_write(rbd_image_t image, uint64_t off, size_t len,
                                   ...
                                   rbd_completion_t c, int op_flags)
{
  ...
  ictx->io_work_queue->aio_compare_and_write(get_aio_completion(comp), off, len,
                std::move(cmp_bl), std::move(bl), op_flags, false);
...
extern "C" ssize_t rbd_aio_get_mismatch_offset(rbd_completion_t c)
{
  librbd::RBD::AioCompletion *comp = (librbd::RBD::AioCompletion *)c;
  return comp->get_mismatch_offset();
}
